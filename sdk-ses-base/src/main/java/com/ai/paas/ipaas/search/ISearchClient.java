...
public interface ISearchClient {
    ...
    /**
     * 按照条件查询，age:(>=10 AND <50) | age:(>=10 AND <50) AND name:1234 具体语法请参见：https
     * ://www.elastic.co/guide/en/elasticsearch/reference/current/query
     * -dsl-query-string-query.html
     * 
     * @param querySQL
     ...
     * @param typeGetter      类型获取器，主要用来支持泛型类型，使用方式：TypeGetter<List<String>>
     *                        typeGetter=TypeGetter<List<String>>() {};
     *                        其中里面为泛型的具体使用方式
     * @return
     */
    public <T> Result<T> search(List<SearchCriteria> searchCriterias, int from, int offset, @Nullable List<Sort> sorts,
     ...
     * @param json
     * 
     * @param addDynamicTemplate 是否增加动态模板
     * 
     *                           <pre>
     * {
     *   "userInfo" : {
