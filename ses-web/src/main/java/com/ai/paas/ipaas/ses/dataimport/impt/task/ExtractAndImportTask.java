...
private static final Logger log = LoggerFactory.getLogger(ExtractAndImportTask.class);
	...
	public ExtractAndImportTask(final TableRuleConfig.LogicDb logicDb,
			...
			CountDownLatch exImLatch,Result result,
			ISearchClient is){
	...
	private void readData() {
        ...
        if(totalNum>0){
	        ...
	        log.debug(" totalNum {},threadNum {},pageSize {}", totalNum,threadNum,pageSize);
	        
	        ExecutorService executorService = new ThreadPoolExecutor(0,threadNum, 10L, TimeUnit.SECONDS,
					new SynchronousQueue<Runnable>());
			final CountDownLatch taskLatch = new CountDownLatch(threadNum);
	
			for(int i=0;i<threadNum;i++){
				executorService.execute(new ExtractImTask(totalNum,threadNum,pageSize,
						i,sql,oSql,taskLatch));
			}
...
//		totalNum = 3003;
		...
		for(int i=0;i<threadNum;i++){
			executorService.execute(new ExtractImTask(totalNum,threadNum,pageSize,
					i,sql.toLowerCase(),sql,taskLatch));
		}
		log.debug("wait for ExtractImTask thread finished");
		try {
			taskLatch.await();
	...
	private class ExtractImTask implements Runnable{
		...
		public void run() {
			try {
				...
				log.error("--------------ExtractImTask excute exception:"+e.getMessage(),e);
			}
			taskLatch.countDown();
		}
		...
		public void excute(
				int totalNum, int threadNum, int rowNum, int i, String sql,String oSql) throws Exception{
			Connection conn = null;
			PreparedStatement statement = null;
			ResultSet rs = null;
			try {
	            ...
	            if(conn!=null){
	            	String preSql = getSql(rowNum, threadNum,totalNum,i,sql,oSql);
				...
				try {
				    		...
				    		if(ec.getAfFiledSqls().containsKey(af)){
				    			...
				    			for(SesIndexFiledSql fs:fss){
				    				...
				    				if(fs.isMapObj()){
										...
										getFiledValue(datas,fs,cn,cv,af,null);
									}
				    			}
				    		}
							datas.put(rs.getMetaData().getColumnLabel(j+1), cv);
			            }
						log.debug("-----excute-------------datas-{}------",datas);
						
						packDatas(datas,ec.getLineMapParams());
						log.debug("-----excute------packDatas-------datas-{}------",datas);
					...
					try {
						...
						log.error("--------------addExcLogs exception:"+ee.getMessage(),ee);
					}
				}
			...
			if (lineMapParams != null && lineMapParams.size() > 0) {
				for (LineMapParam l : lineMapParams) {
					if (l.getPaf() == null || "".equals(l.getPaf())) {
						Object obj = l.getRs();
						if (l.isMany() && (obj!=null) && obj instanceof ArrayList) {
							...
							while (getChildFiled(af,parentAlias) != null) {
								LineMapParam child = getChildFiled(af,parentAlias);
								...
								if (child.isMany()
									...
									childList = (ArrayList) child
											.getRs();
									int counts = 0;
									if(parentflists!=null&&parentflists.size()>0){
										for(int i=0;i<parentflists.size();i++){
											HashMap flistMap = (HashMap)parentflists.get(i);
											List<Map> fileds = new ArrayList<Map>();
											for( ;counts<childList.size();counts++){
												
												if(((HashMap)childList.get(counts)).containsKey("flagBreak")){
													if(counts==0)
														continue;
													counts++;
													break;
												}
												fileds.add((HashMap)childList.get(counts));
											}
											flistMap.put(getLocalFiled(child.getAlias()),fileds);
									...
									if(parentflists!=null&&parentflists.size()>0)
										((HashMap)parentflists.get(0)).put(getLocalFiled(child.getAlias()), childmap);
								}
								af = child.getAf();
						...
						} else if(!l.isMany()&&(obj!=null) && obj instanceof HashMap){
							...
							while (getChildFiled(af,parentAlias) != null) {
								...
								if (child.isMany()
									...
									parentMap.put(getLocalFiled(child.getAlias()), fileds);
								} else if (!child.isMany()
										&& child.getRs() instanceof HashMap) {
									HashMap childmap = (HashMap) child.getRs();
									parentMap.put(getLocalFiled(child.getAlias()), childmap);
		...
		private LineMapParam getlineBean(String af,String alias,boolean start,boolean newBreak) {
		...
		private String getSql(int rowNum, int threadNum,int totalNum,int i,String sql,String oSql) {
			int start = i * rowNum;
			int rowNum2 = rowNum;
			if (i == threadNum - 1) {
			...
			try {
				...
				while (resultSet.next()) {
					...
					for (int i = 0; i < resultSet.getMetaData().getColumnCount(); i++) {
						String rcn = resultSet.getMetaData().getColumnLabel(i + 1)
								.toLowerCase();
						...
						if (many) {
							...
							if(!fs.isMapObj())
								datas.put(resultSet.getMetaData().getColumnLabel(i+1), rcv);
							if(line!=null)
								line.put(resultSet.getMetaData().getColumnLabel(i+1), rcv);
...
//									getFiledValue(datas,ffs,rcn,rcv,saf,getlineBean(saf,ffs.getAlias(),nextNewLine,false));
									getFiledValue(datas,ffs,rcn,rcv,saf,getlineBean(saf,ffs.getAlias(),nextNewLine,true));
				    			}
				    		}
						}else{
							if (ec.getPafAfs().containsKey(saf)) {
								...
								for (String tempAf : afs) {
									getFiledValue(datas,
											ec.getFiledSql().get(tempAf.substring(0,tempAf.lastIndexOf("."))),
											rcn, rcv, saf,null);
								}
							}
						}
...
//								.toLowerCase();
						...
						if(ec.getTableFileds().containsKey(fs.getIndexAlias() + "."+rcn)){
...
//						final String saf = ec.getTableFileds().containsKey(fs.getIndexAlias() + "."+rcn)?
//								(fs.getIndexAlias() + "." +ec.getTableFileds().get(fs.getIndexAlias() + "."+rcn)):
