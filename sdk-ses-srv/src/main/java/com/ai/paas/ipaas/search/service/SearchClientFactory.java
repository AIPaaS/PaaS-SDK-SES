...
//	private static ISearchClient iSearchClient;
	private static transient final Logger log = LoggerFactory.getLogger(SearchClientFactory.class);
	...
	public  static ISearchClient getSearchClient(AuthDescriptor ad) throws Exception {
...
//		// 获取该用户申请的cache服务配置信息
		...
		String personalConf = CCSComponentFactory.getConfigClient(authResult.getConfigAddr(), 
				authResult.getConfigUser(),authResult.getConfigPasswd()).get(SEARCH_CONFIG_PATH+srvId);
		
		String mappingConf = CCSComponentFactory.getConfigClient(authResult.getConfigAddr(), 
				authResult.getConfigUser(),authResult.getConfigPasswd()).get(SES_MAPPING_ZK_PATH+srvId,watch);
		
		Gson gson = new Gson();
		Map<String,Object> personalConfMap = new HashMap<String,Object>();
		Map<String,Object> mappingConfMap = new HashMap<String,Object>();
		personalConfMap = gson.fromJson(personalConf, personalConfMap.getClass());
		
		
		mappingConfMap = gson.fromJson(mappingConf, mappingConfMap.getClass());
		String indexName = String.valueOf(Math.abs((authResult.getUserId()+srvId).hashCode()));
		String hosts = String.valueOf(personalConfMap.get(ELASTIC_HOST));
		
		
		Map map = gson.fromJson(mappingConf, Map.class);
		JsonObject properties = gson.fromJson((String)map.get("mapping"),JsonObject.class).get(indexName).getAsJsonObject().get("properties").getAsJsonObject();
		
		JsonObject idObj = gson.fromJson((String)map.get("mapping"),JsonObject.class).get(indexName).getAsJsonObject().get("_id").getAsJsonObject();
		idObj.get("path").toString();
		iSearchClient = new SearchClientImpl(hosts,indexName,properties,idObj);
	...
	private static class SesWatch extends ConfigWatcher{
		...
		public SesWatch(ICCSComponent client,String userPid,String userId,String serviceId){
			this.client = client;
		...
		public void processEvent(ConfigWatcherEvent event) {
			...
			if (EventType.NodeDataChanged == eventType){
				...
				try{
					...
					String personalConf =client.get(SEARCH_CONFIG_PATH+serviceId,this);
					String mappingConf = client.get(SES_MAPPING_ZK_PATH+serviceId,this);
					...
					personalConfMap = gson.fromJson(personalConf, personalConfMap.getClass());
					mappingConfMap = gson.fromJson(mappingConf, mappingConfMap.getClass());
					String indexName = String.valueOf(Math.abs((userId+serviceId).hashCode()));
					String hosts = String.valueOf(personalConfMap.get(ELASTIC_HOST));
